...
3. 每次改动后至少执行：
   ...
   - 若需要执行 `go test ./...` 但当前环境没有 `go` 命令，优先使用带缓存目录挂载的 Docker 容器执行测试，避免每次重新下载依赖
   - 容器执行 Go 命令时，优先复用 Go module cache 与 build cache（例如挂载 `GOMODCACHE`、`GOCACHE` 对应目录或等效缓存卷）
...
- 若本地缺少 `go` 命令，则应优先通过带缓存目录挂载的 Docker 容器完成 `go test ./...`
