...
CREATE TABLE IF NOT EXISTS master_data_sync_status_new (
...
INSERT INTO master_data_sync_status_new (
  region,
  status,
  file_count,
  sync_duration_ms,
...
ALTER TABLE master_data_sync_status_new RENAME TO master_data_sync_status;
...
FROM (
  SELECT
    ...
    ROW_NUMBER() OVER (
      PARTITION BY region
      ORDER BY created_at DESC, last_synced_at DESC
    ) AS row_num
  FROM master_data_sync_status
) latest
