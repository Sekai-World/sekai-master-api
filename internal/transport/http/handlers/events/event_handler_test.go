...
func TestEventListEndpointFiltersByMultipleBannerGameCharacterUnits(t *testing.T) {
	gin.SetMode(gin.TestMode)
	...
	cache := &fakeEventHandlerCache{
		byID: map[string]map[string]map[string]map[string]any{
			"jp": {
				"gamecharacterunits": {
					"18": {"id": 18, "gameCharacterId": 6, "unit": "idol"},
					"19": {"id": 19, "gameCharacterId": 7, "unit": "street"},
					"20": {"id": 20, "gameCharacterId": 8, "unit": "light_sound"},
				},
				"gamecharacters": {
					"6": {"id": 6, "firstName": "Haruka", "givenName": "Kiritani"},
					"7": {"id": 7, "firstName": "Kohane", "givenName": "Azusawa"},
					"8": {"id": 8, "firstName": "Ichika", "givenName": "Hoshino"},
				},
			},
		},
		listByEntity: map[string]map[string][]map[string]any{
			"jp": {
				"events": {
					{"id": 101, "name": "Idol Banner", "eventType": "marathon"},
					{"id": 102, "name": "Street Banner", "eventType": "marathon"},
					{"id": 103, "name": "Light Sound Banner", "eventType": "marathon"},
				},
				"eventstories": {
					{"id": 7001, "eventId": 101, "bannerGameCharacterUnitId": 18},
					{"id": 7002, "eventId": 102, "bannerGameCharacterUnitId": 19},
					{"id": 7003, "eventId": 103, "bannerGameCharacterUnitId": 20},
				},
			},
		},
	}
	...
	req := httptest.NewRequest(http.MethodGet, "/api/v1/events/jp/list?banner_game_character_unit=idol&banner_game_character_unit=light_sound&page=1&page_size=20", nil)
	resp := httptest.NewRecorder()
	router.ServeHTTP(resp, req)
	...
	assertResponseItemOrder(t, resp.Body.Bytes(), []float64{101, 103})
}

func TestEventListEndpointBannerGameCharacterUnitFilterRequiresExactUnitCode(t *testing.T) {
	gin.SetMode(gin.TestMode)
	...
	cache := &fakeEventHandlerCache{
		byID: map[string]map[string]map[string]map[string]any{
			"jp": {
				"gamecharacterunits": {
					"18": {"id": 18, "gameCharacterId": 6, "unit": "idol"},
					"19": {"id": 19, "gameCharacterId": 7, "unit": "virtual_singer_idol"},
				},
			},
		},
		listByEntity: map[string]map[string][]map[string]any{
			"jp": {
				"events": {
					{"id": 101, "name": "Exact Banner", "eventType": "marathon"},
					{"id": 102, "name": "Contains Banner", "eventType": "marathon"},
				},
				"eventstories": {
					{"id": 7001, "eventId": 101, "bannerGameCharacterUnitId": 18},
					{"id": 7002, "eventId": 102, "bannerGameCharacterUnitId": 19},
	...
	req := httptest.NewRequest(http.MethodGet, "/api/v1/events/jp/list?banner_game_character_unit=idol&page=1&page_size=20", nil)
	resp := httptest.NewRecorder()
	router.ServeHTTP(resp, req)
...
func TestEventListEndpointFiltersByCommaSeparatedBannerGameCharacterUnits(t *testing.T) {
	gin.SetMode(gin.TestMode)
	...
	req := httptest.NewRequest(http.MethodGet, "/api/v1/events/jp/list?banner_game_character_unit=idol,light_sound&page=1&page_size=20", nil)
	resp := httptest.NewRecorder()
	router.ServeHTTP(resp, req)
