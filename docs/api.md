...
Mission endpoints allow only the explicit `storyMissions`, `characterMissionV2s`, and `normalMissions` families. They use typed family-specific projections and omit unknown upstream fields. Lists support positive `id` and `event_id` filters; `character_id` is supported only for `characterMissionV2s`. `sort_by` and `sort_order=asc|desc` are validated per family, and pagination is applied after normalization. Reward references are typed and keep `status: unresolved` when a resource-box purpose or unique same-region relationship cannot be established. Resource-box IDs are shared across purposes, so a reference without its own purpose prefers the family's purpose: story missions (which store only a top-level `resourceBoxId`) use `story_mission`, reward entries whose `missionType` ends in `_mission` use `mission_reward`, and normal mission rewards stored as bare box-ID groups (TW/KR/CN `[[5]]`) also use `mission_reward`; when no box of that purpose matches, the ID-only match still applies. Character ranks also carry `totalExp`, the cumulative character rank EXP to reach that rank, from the same-region `levels` records with `levelType: character` (JP 175 ranks, other regions 160; KR values differ). Where a resource box does not embed `details` (TW/KR/CN), mission and character-rank rewards join them from `resourceboxdetails` by `(resourceBoxPurpose, resourceBoxId)`. Reward details for gacha tickets, materials, skill practice tickets, and boost items also carry the item's same-region `resourceName` (the same flat fields appear on event ranking and virtual live reward details), joined from `gachatickets`, `materials`, `skillpracticetickets`, and `boostitems` by `resourceId`; gacha ticket details add `resourceAssetbundleName`, which their icon path (`thumbnail/gacha_ticket/{assetbundleName}.webp`) needs. Resource boxes, details, and these item entities are batch-loaded with `ListAll` only, and the decoded indexes are reused per process while each entity revision is unchanged (see [Master Data](master-data.md#cache-strategy)); no cross-region fallback or by-id lookup is used.
